...
<?php       
  ...
  ?></a><span class="comma">,</span>
    ...
    { // If OK
        ...
        $query_select_guid = mysql_query("SELECT item_template FROM character_inventory WHERE guid = '$guid' AND bag='0' AND slot <'18'");
        if (mysql_num_rows($query_select_guid) != 0) 
        {
            ...
            while($row_select_guid = mysql_fetch_array($query_select_guid))
            {
                ...
                $query_select_itemEntry = mysql_query("SELECT item_template FROM item_inventory WHERE guid = '$item'");
                if (mysql_num_rows($query_select_itemEntry) != 0) 
                {
                    while ($row_itemEntry = mysql_fetch_array($query_select_itemEntry)) 
                    {
                        $itemEntry = $row_itemEntry['item_template'];
                        if ($itemEntry != "") 
                        {
