...
if(isset($_POST['submit']))
{
	...
	//查询金币数量
	$buscarpuntosMoney = mysql_fetch_array(mysql_query("SELECT * FROM $server_cdb.characters WHERE guid = '".$character."' AND account = '".$account_information['id']."'"));
	//结束
	...
	if(empty($character) || mysql_num_rows($check) < 1) $errors[] = "你没有选择一个合适的游戏角色进行更改.";
	
	if ($buscarpuntosMoney[$method1]<$price[$method]) $errors[] = "您的金币不足，无法支付。 ".$method2;

	if(count($errors) < 1){
		$total=$buscarpuntosMoney[$method1]-$price[$method];
		$substract = mysql_query("UPDATE $server_cdb.characters SET $method1=$total WHERE id='".$account_information['id']."'");
		$change = mysql_query("UPDATE $server_cdb.characters SET at_login = '128' WHERE guid = '".$character."'");
		echo '<p align="center"><font color="green"><strong>成功!</strong></font><br/>';
		echo "<strong>已经成功的更改了您的阵营，请登录游戏查看.</strong>";
		echo '</p>';
		echo '<meta http-equiv="refresh" content="2;url=../account_man.php"/>';

	}else{
	  echo '<p align="center"><font color="red"><strong>错误！</strong></font><br/>';
...
?>
	...
	if($free==0){
		echo '<div class="form-row required">
			...
			<select id="type" name="type">';
				if ($price['dp'] > 0) echo '<option value="1">衣米在线积分</option>';
				if ($price['vp'] > 0) echo '<option value="2">游戏金币</option>';
			echo'</select>
		</div>';
	} ?>
	<fieldset class="ui-controls " >
      ...
      echo '*你没有一个可以选择的角色，或是您的角色无法满足变更条件.<br><br>
      <button class="ui-button button1 disabled" type="submit" name="submit" id="settings-submit" value="Continue" tabindex="1" disabled="disabled">';
		...
		elseif($online == 1) echo '*您的一个角色正在游戏中，请退出游戏再进行操作。<br><br><button class="ui-button button1 disabled" type="submit" name="submit" id="settings-submit" value="Continue" tabindex="1" disabled="disabled">';
...
ticketStatus: 'Ticket {0}抯 status changed to爗1}.',
ticketOpen: 'Open',
