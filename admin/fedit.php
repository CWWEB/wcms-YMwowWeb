    ...
    <div id="content">
      <div class="forms">
        ...
        <table width="100%" class="table-content">
          <tr>
            <td width="50%"><p><strong>锁定状态: </strong><?php if($forum['locked']==1) echo '<font color="red">锁定</font>'; else echo '<font color="green">解锁</font>';?></p></td>
            <td>
              <p>
                <strong>父版块: </strong>
                <select name="f_categ">
                ...
                <button type="submit" name="s_categ">更改</button>
              </p>
            </td>
          </tr>
          <tr><td>
          ...
          <td><p><strong>删除版块: </strong>
            <select name="f_delete" id="delete">
            <option value="del_all" selected="selected">删除所有版块</option>
            <?php
              while($op = mysql_fetch_assoc($all_forum)){
                if($op['categ'] != $last_categ) echo '<optgroup label="移动至 '.$array_categ[$op['categ']].'">';
                echo'<option value="'.$op['id'].'" name="'.$op['name'].'" id="op_del'.$op['id'].'">'.$op['name'].'</option>';
                $last_categ = $op['categ'];
              }
              ?>
            </select> 
            <button type="submit" name="s_delete" onclick="return confirm_del();">删除</button>
          </p></td></tr>
          <tr><td><p><strong>名称: </strong>
          <input type="text" name="f_name" value="<?php echo $forum['name']; ?>" class="reg" /></p></td>
          <td></td></tr>
          <tr><td><p><strong>简介: </strong><br />
