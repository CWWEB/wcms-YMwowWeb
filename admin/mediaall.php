...
<body class="bgc">
    ...
    <div id="content">
        ...
        <form method="get" action="">
          <select name="sort" onchange="submit(this.form)">
            <option value="" <?php if(!isset($_GET['sort'])){echo 'selected="selected"';} ?>>排序</option>
            <option value="date" <?php if($_GET['sort']=='date'){echo 'selected="selected"';} ?>>日期</option>
            <option value="type" <?php if($_GET['sort']=='type'){echo 'selected="selected"';} ?>>类型</option>
            <option value="title" <?php if($_GET['sort']=='title'){echo 'selected="selected"';} ?>>标题</option>
            <option value="author" <?php if($_GET['sort']=='author'){echo 'selected="selected"';} ?>>作者</option>
          </select>
          <select name="type" onchange="submit(this.form)">
            <option value="all" <?php if(!isset($_GET['type']) || $_GET['type']=='all'){echo 'selected="selected"';} ?>>全部</option>
            <option value="0" <?php if($_GET['type']=='0'){echo 'selected="selected"';} ?>>视频</option>
            <option value="1" <?php if($_GET['type']=='1'){echo 'selected="selected"';} ?>>壁纸</option>
            <option value="2" <?php if($_GET['type']=='2'){echo 'selected="selected"';} ?>>电影</option>
            <option value="3" <?php if($_GET['type']=='3'){echo 'selected="selected"';} ?>>艺术</option>
            <option value="3" <?php if($_GET['type']=='4'){echo 'selected="selected"';} ?>>动漫</option>
          </select>
        </form>
      </div>
      ...
      <table>
        ...
        <tr>  
          ...
          <th class="edit"><strong>撤销/删除</strong></th>   
          <th class="title"><strong>标题</strong></th>
          <th class="desc"><strong>简介</strong></th>
          <th class="inc"><strong>作者</strong></th>
          <th class="inc"><strong>日期</strong></th>
          <th class="inc"><strong>类型</strong></th>
        </tr>
        </thead>
        <tbody>
      <?php
    ...
    <img src="images/sepLine.png" alt="" class="sepline" />
        ...
        <div><img src="images/warningIco.png" alt="" />
                  <p>警告信息, Lorem ipsum dolor sit amet, consectetur adipiscing elit Pellentesque quis.</p>
                </div>
        <div><img src="images/infoIcon.png" alt="" />
                  <p>信息, Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
                </div>
        <div><img src="images/success.png" alt="" />
                  <p>成功信息, Lorem ipsum dolor sit amet, Nam bibendum sagittis lobortis.consectetur.</p>
                </div>
        <div><img src="images/errorIco.png" alt="" />
                  <p>错误消息, Lorem ipsum dolor sit amet, Nam bibendum sagittis lobortis.consectetur.</p>
                </div>
      </div> -->
              <div id="calen">
