<?php
require_once("configs_pve.php");
require("functions/armory_func_pve.php");
...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
 <html lang="en-gb">
<head>
<title>搜索 - <?php echo $website['title']; ?></title>
...
<li class="last children"><a href="search_pve.php" rel="np"><?php echo $Ind['Ind2']; ?></a>
</li>
</ol>
</div>
        	...
        	<div class="search">
          	<div class="search-left">
            	...
            	if (!$error){
            	 echo '<ul class="dynamic-menu" id="menu-search">
              	...
              	if ($num_char>0){ echo '<li><a href="search_c_pve.php?search='.$term.'"><span class="arrow">'.$status['chars'].' ('.$num_char.')'.'<span></span></span></a></li>';}
	              if ($num_guild>0){ echo '<li><a href="search_g_pve.php?search='.$term.'"><span class="arrow">'.$guild['Guilds'].' ('.$num_guild.')'.'<span></span></span></a></li>';}
               	if ($num_arena>0){ echo '<li><a href="search_a_pve.php?search='.$term.'"><span class="arrow">'.$arena['Teams'].' ('.$num_arena.')'.'<span></span></span></a></li>';}
               	if ($num_forum>0){ echo '<li><a href="search_f_pve.php?search='.$term.'"><span class="arrow">'.$Forums['Forums'].' ('.$num_forum.')'.'<span></span></span></a></li>';}
            	 echo '</ul>';} ?>
            </div>  
              ...
              if (!$error && $num_char > 0){
                ...
                echo '<div class="results results-grid wow-results">
	                   <h3 class="category "><a href="search_c_pve.php?search='.$term.'">'.$status['chars'].'</a> ('.$num_char.')</h3>';
                while ($row = mysql_fetch_array($result)) {
                  echo'
                    <div class="grid">
	                    <div class="wowcharacter">
	                      <a href="advanced_pve.php?name='.$row['name'].'" class="icon-frame frame-56 thumbnail">
	                      <img src="images/avatars/2d/'.$row['race'].'-'.$row['gender'].'.jpg" alt="" width="56" height="56" /></a>
	                      <a href="advanced_pve.php?name='.$row['name'].'" class="color-c'.$row['class'].'">
               ...
               if (!$error && ($num_arena || $num_guild)){
                  ...
                  if ($num_arena > 0){
                      ...
                      echo'<div class="grid">
	                     <h4 class="subcategory "><a href="search_a_pve.php?search='.$term.'">'.$arena['Teams'].'</a> ('.$num_arena.')</h4>
					  ...
					  echo'
					<div class="grid">
					<h4 class="subcategory "><a href="search_g_pve.php?search='.$term.'">'.$guild['Guilds'].'</a>('.$num_guild.')</h4>
					<div class="wowguild">
               ...
               if (!$error && $num_forum > 0){
                  ...
                  $result= mysql_query($sql,$conn) or die (mysql_error());
                  echo '<div class="results results-grid wow-results">
                  <h4 class="subcategory "><a href="search_f_pve.php?search='.$term.'">'.$search['forumResults'].$term.'</a> ('.$num_forum.')</h4>
                  <div class="view-list">';
                    while ($row = mysql_fetch_array($result)){
                      $content=$row['content'];
